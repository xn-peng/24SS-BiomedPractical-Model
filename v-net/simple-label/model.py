...
def average_pooling_downsample(in_channels, out_channels):
    return nn.Sequential(
        nn.AvgPool3d(kernel_size=2, stride=2),  # 使用3D平均池化进行降采样
...
class VNet(nn.Module):
    def __init__(self, use_downsampling=False):
        ...
        self.initial_downsample = nn.Sequential(
            nn.AvgPool3d(kernel_size=2, stride=2),  # 从256x256x256降至128x128x128
    ...
    def forward(self, x):
        if self.use_downsampling:
            x = self.initial_downsample(x)  # Apply initial downsampling
        # Processing layers
        ...
        if self.use_downsampling:
            deconv = self.final_upsample(deconv)  # Upsample to original size if downsampling was used
...
class CombinedVNetModel(nn.Module):
    def __init__(self, num_extra_features, use_downsampling=False):
        ...
        self.extra_features = ExtraFeatures(num_extra_features, 32)  # Adjust the number of output channels as needed
        self.final_conv = nn.Conv3d(32, 1, kernel_size=1)  # This converts combined features to the final output channel
    ...
    def forward(self, x, extra_features):
        ...
        output = torch.sigmoid(self.final_conv(combined_output))  # sigmoid to get values between 0 and 1
        return output